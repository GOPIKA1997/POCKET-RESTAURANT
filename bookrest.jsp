...
$(document).ready(function() {
	$('#approveHeadingRow').html("");
var curr_date=formatDate(new Date());
		alert("bookrest_json.jsp?curr_date="+curr_date)
		$.getJSON("bookrest_json.jsp?curr_date="+curr_date, function(json){
			$('#approveHeadingRow').html("");
			var len=json.length;
			var i=0;
			if(json.length==0)
			...
			while(i<len)
				{
					var table=json[i].table_num;
					var name=json[i].name;
					//alert(name);
					var userreg_id=json[i].userreg_id;
					var buy_id=json[i].buy_id;
					$("#approveHeadingRow").append('<tr><td>'+(i+1)+'</td><td>'+table+'</td><td>'+name+
							'</td><td><a href="bookrest_more.jsp?userreg_id='+userreg_id+'&buy_id='+buy_id+'"><input type="submit" value="VIEW" class="btn btn-sm btn-danger"></a></td></tr>');
					i++;
		 		}
